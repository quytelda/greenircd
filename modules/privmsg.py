...
def handle_event(srv, ctcn, params):
	if len(params) < 2:
		srv.send_msg(user, "461 %s PRIVMSG :PRIVMSG takes at least 2 parameters!" % user.nick)
		return
	...
	if target in srv.channels: # this is a channel message
		...
		srv.send_msg(srv.clients[target], 'PRIVMSG %s :%s' % (target, message), ctcn.get_hostmask())
	else:
		return
